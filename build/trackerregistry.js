(function(){/* ===================================================================
   TRACKER REGISTRY — Unified Journal Tracker · Phase 1 (data only)
   -------------------------------------------------------------------
   Single machine-readable source of truth describing every module →
   tracker → section → item (question / tool / log / graphic) that
   exists in the app TODAY, including Flare Mode's micro-logging items
   (mapped READ-ONLY — their components are never edited) and the
   ...
   Phase 2's resolveUnifiedTracker). Nothing existing was modified.

   Conventions:
   • dataPath uses the app's real storage: localStorage keys
     (auto-namespaced per account via the cfns shim). `[d]` = the
     'YYYY-MM-DD' day key inside a per-day store.
   • i18nKey = the English tr() key exactly as the source component
     uses it (the app's sentence-key i18n convention).
   • canonicalKey marks items that are the SAME measure across
     trackers/modes. status 'shared-live' = already unified today
     (single field or existing flaresync/healthsync bridge).
