(function(){/* ===================================================================
   ...
   as the stock/alarm engines; 0 = due today).
   MED22 (8 Sep 2026): the arithmetic moved to medstate.jsx
   (cfMedDaysUntilDue), the single home of the period, so the «next dose»
   resolver and this bridge can never disagree about which day a weekly
   injection belongs to. The local copy stays as the guarded fallback — same
   pattern as alDueToday → cfMedDueToday in medalarm.jsx. */function mbDaysUntilDue(m){if(window.cfMedDaysUntilDue){const d=cfMedDaysUntilDue(m);if(typeof d==='number')return d;}if((m.period||'day')==='day')return 0;const interval=MB_PERIOD_DAYS[m.period]||1;return Math.max(0,Math.ceil(interval-1-(Number(m.acc)||0)*interval-1e-9));}/* MED3/MED5: re-avisos vivos escritos por medalarm.jsx — el móvil los tiene que
...
const isToday=dk===mbTodayKey();let delta=0;if(isToday&&action==='taken')delta=-units;else if(!isToday&&action==='skipped'&&(med.period||'day')==='day')delta=+units;if(!delta)return;try{localStorage.setItem('cf_meds_v3',JSON.stringify(meds.map(m=>m.id===medId?{...m,stock:Math.max(0,(Number(m.stock)||0)+delta)}:m)));}catch(e){}}/* 🔴 The diary offer that used to fire here is GONE (7 Sep 2026).
   A dose confirmed FROM THE PHONE NOTIFICATION opened the same blue
   «Same routine as yesterday?» window as the in-app alarm — the door
   that escapes anyone who only greps for the component name. Confirming
   a dose from the notification now does exactly what it says it does:
   it records the dose. The day is asked for by the evening reminder,
   one question per screen (checkinflow.jsx). *//* ---- the glue ---- *//* explicit per-dose cancel — mbHorizon only carries FUTURE doses, so a dose
